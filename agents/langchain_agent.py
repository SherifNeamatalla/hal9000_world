...
class LangchainAgent(BaseAgent):
    ...
    def create_context(self, user_input):
        ...
        # short_term_result = create_langchain_short_term_memory_context(self.config.get('model'),
        #                                                                self.config.get('max_tokens'), context,
        #                                                                user_input,
    ...
    def create_long_term_memory_context(self):
        ...
        return [create_langchain_message(SYSTEM_ROLE, message),
                create_langchain_message(SYSTEM_ROLE, self.long_term_memory.get_as_string())]
    ...
    def add_prebuilt_langchain_tools(self):
        # TODO load from config
        # prebuilt_tools = load_tools(["google-serper"], llm=self.llm)
        prebuilt_tools = []

        self.langchain_tools.extend(prebuilt_tools)
